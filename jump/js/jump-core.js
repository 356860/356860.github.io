(function (global) {
    ...
    function pointVisible(point, minVisibility = JUMP_CONFIG.minVisibility) {
    ...
    function smoothFeatures(previous, features, alpha = JUMP_CONFIG.emaAlpha) {
        if (!previous) {
            return {
                ...features,
    ...
    function createAttemptState(baseline) {
        return {
    ...
    function buildTopReasonText(bucket, issueMeta = ISSUE_META) {
    ...
    function buildRoundAnalysis(round, mode, issueMeta = ISSUE_META) {
        const practiceCount = Number(round.practiceCount || 0);
        const successCount = Number(round.successCount || 0);
